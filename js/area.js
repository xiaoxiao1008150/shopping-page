...
var k=["s_province1","s_city1","s_county1"];//三个select的name
// var opt01 = ["省","市","区/县"];//初始值
function _init_area1(){  //初始化函数
	for(i=0;i<k.length-1;i++){
		document.getElementById(k[i]).onchange=new Function("change1("+(i+1)+")");
	}
	change1(0);
}
...
function change1(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(k[i]).selectedIndex-1));
	}
	var ss=document.getElementById(k[v]);
	with(ss){
		length = 0;
		options[0]=new Option(opt0[v],opt0[v]);
		if(v && document.getElementById(k[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
